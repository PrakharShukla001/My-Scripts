...
# ⚠️  IP WARNING
# EC2 restart   → Public IP badlegi (Private same rehti hai)
# Naya instance → Private IP bhi badal sakti hai
# FIX: AWS Console → EC2 → Elastic IPs → Allocate → Associate → Master instance

banner() {
  echo -e "${CYAN}"
  echo "╔══════════════════════════════════════════════╗"
  echo "║   ☸️   Kubernetes — MASTER NODE (CentOS 7)   ║"
...
EOF
  ...
  log_step "Step 10 — kubeadm init (Control Plane)"
  log_info "Master IP : ${MASTER_IP}"
  ...
  # ── Step 11: kubectl config ──────────────────────────────────────────────
  log_step "Step 11 — Configure kubectl"
  mkdir -p "$HOME/.kube"
  ...
  # ── Step 12: Calico CNI ──────────────────────────────────────────────────
  log_step "Step 12 — Install Calico CNI"
  ...
  echo -e "${GREEN}${BOLD}║        ✅  Master Node Ready! (CentOS 7)      ║${RESET}"
  echo -e "${GREEN}${BOLD}╚══════════════════════════════════════════════╝${RESET}"
  echo ""
...
# ─── REMOVE MASTER ────────────────────────────────────────────────────────────
remove_master() {
  log_step "Master Node Remove (CentOS 7)"
